  ...
  return (
    <section className="mx-auto max-w-[var(--container-page)] px-gutter py-24">
      <SectionHeader
        ...
        title={hidden ? "This month: open for orders" : `This month: ${currentEdition.subject}`}
        ink="vermilion"
      />
      <p className="mono-label mt-4">
        {hidden
          ? `${currentEdition.month} · order now — the country is revealed in the envelope`
      ...
      <div className="mt-14 grid gap-12 border-t border-hairline pt-10 lg:grid-cols-[1.35fr_1fr]">
        <div className="max-w-[var(--container-measure)]">
          <h3 className="font-serif-display text-2xl">
            {hidden ? `Order now, open it in ${monthName}` : `${currentEdition.subject}, drawn from the ground`}
          </h3>
          <p className="mt-5 text-[1.02rem] leading-relaxed text-ink-soft">
            {hidden
              ? `The ${currentEdition.month} edition is open for orders now. Which country it is stays a surprise until the envelope arrives.`
              : currentEdition.note}
          </p>
        ...
        {/* the shop moment: price and a real add-to-cart */}
